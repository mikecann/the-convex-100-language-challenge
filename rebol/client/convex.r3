...
;; ===========================================================================
;; Randomness and UUIDs. Building raw bytes from `random/secure 256` needs
;; `to binary! reduce [n]` rather than `to-char n` + append: TO-CHAR treats
;; a value above 127 as a Unicode code point and a binary APPEND of a
;; char! UTF-8-encodes it, silently expanding a single byte 0x80-0xFF into
;; two or more -- confirmed directly by probing this build. `to binary!
;; reduce [n]` instead produces the exact single raw byte every time.
;;
;; A fresh Rebol/Bulk 3.22.1 process, on its own, is NOT a source of
;; entropy: `random/secure` draws the exact same deterministic byte
;; sequence on every fresh process start until something calls
;; `random/seed`, confirmed directly by running an identical script in
;; three separate containers and getting three identical results. Left
;; unseeded, every room/runId this client generates would collide across
;; every single Docker run, silently breaking the "start at 0" contract
;; example/adapter verification depends on. seed-rng-once reseeds from
;; now/precise's sub-second wall clock exactly once per process, which is
;; enough for this client's own uses of randomness (room/runId uniqueness,
;; WebSocket mask keys, session ids) -- none of those are a cryptographic
;; guarantee this client relies on. The one place cryptographic randomness
;; actually matters, RSA/ECDSA key material inside the TLS handshake
;; itself, is generated by the native tls:// scheme's own internal entropy
;; source rather than this `random` mezzanine action, and is unaffected by
;; this seed either way.
;;
;; `does` (not `function`) so the bare set-words below mutate the module-
;; level `rng-seeded?` rather than shadowing it as a new local -- the same
;; FUNCTION-vs-FUNC/DOES trap this file's header documents.
rng-seeded?: false
seed-rng-once: does [
    unless rng-seeded? [
