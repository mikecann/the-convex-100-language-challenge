...
class TestExample(unittest.TestCase):
    ...
    def test_whole_rejects_values_that_cannot_be_counter_states(self):
        for value in (True, False, 0.5, float('nan'), float('inf'), float('-inf'), '0', None):
            with self.subTest(value=value): self.assertRaises(RuntimeError,whole,value,'test')

    def test_integral_float_payloads_complete_and_cleanup(self):
        class Subscription:
            def __init__(self): self.values=iter([types.SimpleNamespace(value={'count':0.0}, error=None), types.SimpleNamespace(value={'count':1.0}, error=None)]); self.closed=False
        ...
        class Client:
            ...
            def query(self,*args): return types.SimpleNamespace(value={'count':0.0})
            def subscribe(self,*args): return self.subscription
            def mutation(self,*args): return types.SimpleNamespace(value={'applied':True,'state':{'count':1.0}})
            def close(self): self.closed=True
