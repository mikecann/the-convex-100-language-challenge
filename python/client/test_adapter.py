...
class TestAdapterTCP(unittest.TestCase):
    def test_tcp_ndjson_lifecycle_and_optional_ids(self):
        ...
        probe.close()
        env = dict(
            os.environ, ADAPTER_LISTEN=f"127.0.0.1:{port}", PYTHONPATH="/work/client"
        )
        process = subprocess.Popen(
            [sys.executable, "/work/client/tests/conformance/adapter.py"], env=env
        )
        ...
        self.assertNotIn("id", ready)
        self.assertEqual(ready["language"], "python")
