...
class ClosedError(ConvexError):
    pass


class ProtocolError(ConvexError):
    pass


class TransportError(ConvexError):
    pass
...
class _WebSocket:
    ...
    def __init__(self, url, client_version):
        ...
        for line in header_lines:
            if ":" not in line:
                self.close()
                raise TransportError("malformed WebSocket upgrade header")
            name, value = line.split(":", 1)
            key_name = name.strip().lower()
            headers[key_name] = headers.get(key_name, "") + (
                ("," if key_name in headers else "") + value.strip()
            )
        ...
        if (
            ...
            or headers.get("sec-websocket-accept", "").encode("ascii", "ignore")
            != expected_accept
        ):
            self.close()
            raise TransportError("WebSocket upgrade was rejected")
    ...
    def _read_until(self, marker):
        ...
        while marker not in data:
            ...
            if not chunk:
                raise TransportError("WebSocket closed during upgrade")
            data += chunk
        return data
    ...
    def send_json(self, value):
        ...
        size = len(payload)
        head = bytes(
            [0x81, 0x80 | (size if size < 126 else 126 if size < 65536 else 127)]
        )
        if size >= 65536:
    ...
    def _send_control(self, opcode, payload=b""):
        if len(payload) > 125:
            raise ProtocolError("oversized WebSocket control frame")
        mask = os.urandom(4)
        self.sock.sendall(
    ...
    def receive_json(self):
        ...
        size = second & 127
        if first & 0x70:
            raise ProtocolError("WebSocket RSV bits require an unnegotiated extension")
        if second & 0x80:
            raise ProtocolError("server WebSocket frames must not be masked")
        if size == 126:
        ...
        if opcode == 8:
            if len(data) == 1:
                raise ProtocolError("invalid WebSocket close payload")
            self._send_control(8, data)
            raise EOFError
        if opcode == 9:
            self._send_control(10, data)
        ...
        try:
            ...
            raise ProtocolError("WebSocket text was not UTF-8") from error
...
class Subscription:
    ...
    def __init__(self, manager, query_id):
        self.manager, self.query_id, self.updates, self.closed = (
            manager,
            query_id,
            queue.Queue(self.MAX_BUFFERED_UPDATES),
            False,
        )

    def next_update(self, timeout=None):
        try:
            update = self.updates.get(timeout=timeout)
        except queue.Empty:
            raise TransportError("timed out waiting for Live update")
        if isinstance(update, ClosedError):
            raise update
        return update
    ...
    def close(self):
        if not self.closed:
            self.closed = True
            self.manager.unsubscribe(self.query_id)
...
class _LiveManager:
    ...
    def __init__(self, url, version, websocket_factory=_WebSocket):
        ...
        self.subs, self.socket, self.lock, self.closed, self.connection_count = (
            {},
            None,
            threading.RLock(),
            False,
    ...
    def subscribe(self, path, args):
        with self.lock:
            ident = max(self.subs, default=-1) + 1
            sub = Subscription(self, ident)
            self.subs[ident] = (path, args, sub)
            if self.socket:
                self._modify([self._add(ident, path, args)])
            return sub
    ...
    def _add(self, ident, path, args):
        return {"type": "Add", "queryId": ident, "udfPath": path, "args": [args]}

    def _connect(self):
        self.socket = self.websocket_factory(self.url, self.version)
        self.query_version = 0
        self.remote_version = dict(self.INITIAL_VERSION)
    ...
    def _modify(self, mods):
        ...
        self.socket.send_json(
            {
                "type": "ModifyQuerySet",
                "baseVersion": self.query_version,
                "newVersion": self.query_version + 1,
                "modifications": mods,
            }
        )
        ...
        while not self.closed:
            ...
            try:
                with self.lock:
                    ...
                    if has_subscriptions and not self.socket:
                        self._connect()
                ...
                if message.get("type") == "Transition":
                    ...
                    pass
                elif message.get("type") == "TransitionChunk":
                    raise ProtocolError("TransitionChunk assembly is deferred")
                else:
                    raise ProtocolError(
                        f"unexpected Live message {message.get('type')!r}"
                    )
                ...
                if not expected and isinstance(error, ProtocolError):
                    for _, _, sub in current_subscriptions:
                        sub.deliver(Update(error=error, logs=[]))
                time.sleep(0.1)

    def _transition(self, message):
        if message.get("startVersion") != self.remote_version:
            raise ProtocolError("Transition start version does not match local version")
        changed = []
        for mod in message.get("modifications", []):
            ident = mod["queryId"]
            if mod["type"] == "QueryUpdated":
                changed.append(
                    (ident, Update(mod.get("value"), logs=mod.get("logLines", [])))
                )
            elif mod["type"] == "QueryFailed":
                changed.append(
                    (
                        ident,
                        Update(
                            error=FunctionError(
                                mod.get("errorMessage", "query failed"),
                                data=mod.get("errorData"),
                                logs=mod.get("logLines", []),
                            ),
                            logs=mod.get("logLines", []),
                        ),
                    )
                )
            elif mod["type"] != "QueryRemoved":
                raise ProtocolError(f"unknown Transition modification {mod['type']!r}")
        self.remote_version = message["endVersion"]
        with self.lock:
            for ident, update in changed:
                if ident in self.subs:
                    self.subs[ident][2].deliver(update)

    def disconnect_for_adapter(self):
        with self.lock:
            if not self.socket:
                raise TransportError("Live WebSocket is not connected")
    ...
    def close(self):
        ...
        with self.lock:
            ...
            for _, _, sub in self.subs.values():
                sub.deliver(ClosedError("Live subscription is closed"))
...
class Client:
    ...
    def __init__(self, deployment_url, bearer_token=None):
        ...
        if p.scheme not in ("http", "https") or not p.hostname or p.username:
            raise ValueError("Convex deployment URL must be an http(s) URL with a host")
        self.url = deployment_url.rstrip("/")
        self.token = bearer_token or ""
        self.closed = False
        self.live = None
    ...
    def _check(self):
        if self.closed:
            raise ClosedError("Convex client is closed")

    def _call(self, operation, path, args):
        self._check()
        if not isinstance(args, dict) or not path:
            raise ValueError("Convex path and named object arguments are required")
        request = urllib.request.Request(
            self.url + "/api/" + operation,
        ...
        if decoded.get("status") == "success" and "value" in decoded:
            return Result(decoded["value"], decoded.get("logLines", []))
        if decoded.get("status") == "error":
            raise FunctionError(
                decoded.get("errorMessage", "Convex function failed"),
                data=decoded.get("errorData"),
                logs=decoded.get("logLines", []),
            )
    ...
    def subscribe(self, path, args=None):
        self._check()
        self.live = self.live or _LiveManager(self.url, self.VERSION)
        return self.live.subscribe(path, args or {})

    def debug_disconnect_for_adapter(self):
        self._check()
        (
            self.live.disconnect_for_adapter()
            if self.live
            else (_ for _ in ()).throw(
                TransportError("Live WebSocket is not connected")
            )
        )
