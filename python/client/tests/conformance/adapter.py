...
def run(reader, writer):
    ...
    def identified(event, ident):
        if ident is not None: event['id']=ident
        return event
    for line in reader:
        command={}; ident=None
        try:
            command=json.loads(line); ident=command.get('id'); op=command['op']
            if op=='hello': event=identified({'protocolVersion':1,'type':'ready','language':'python','implementation':'native-python-3.13','runtime':sys.version.split()[0]}, ident)
            elif op in ('query','mutation','action'):
                client=client or Client(os.environ['CONVEX_URL'], os.environ.get('CONVEX_AUTH_TOKEN')); result=getattr(client,op)(command['path'],command.get('args',{})); event=identified({'type':'result','value':result.value,'logs':result.logs}, ident)
            elif op=='setAuth': client=client or Client(os.environ['CONVEX_URL']); client.set_auth(command.get('token','')); event=identified({'type':'ack'}, ident)
            elif op=='subscribe':
                ...
                threading.Thread(target=forward,args=(command['subscriptionId'],subscription),daemon=True).start(); event=identified({'type':'ack'}, ident)
            elif op=='unsubscribe': subscriptions.pop(command['subscriptionId']).close(); event=identified({'type':'ack'}, ident)
            elif op=='debugDisconnect': client.debug_disconnect_for_adapter(); event=identified({'type':'ack'}, ident)
            elif op=='close':
                for sub in subscriptions.values(): sub.close()
                if client: client.close()
                emit(identified({'type':'closed'}, ident)); return
            else: raise ValueError(f'unknown adapter operation {op!r}')
        except Exception as exc: event=identified({'type':'error','error':error(exc)}, ident)
        emit(event)
if os.environ.get('ADAPTER_LISTEN'):
    host,port=os.environ['ADAPTER_LISTEN'].rsplit(':',1); server=socket.create_server((host,int(port))); connection,_=server.accept()
    reader=connection.makefile('r'); writer=connection.makefile('w')
    try: run(reader,writer)
    finally: reader.close(); writer.close(); connection.close(); server.close()
