...
class TestWebSocketFrames(unittest.TestCase):
    def handshake_server(self, response_builder, leftover=b''):
        server=socket.create_server(('127.0.0.1',0)); port=server.getsockname()[1]
        def serve():
            connection,_=server.accept(); request=b''
            ...
            connection.sendall(response_builder(accept).encode()+leftover); time.sleep(.1); connection.close(); server.close()
        thread=threading.Thread(target=serve); thread.start(); self.addCleanup(thread.join)
        return f'ws://127.0.0.1:{port}/api/sync'

    def test_constructor_validates_upgrade_and_preserves_first_frame(self):
        message=json.dumps({'type':'Ping'}).encode(); frame=bytes([0x81,len(message)])+message
        url=self.handshake_server(lambda accept: f'HTTP/1.1 101 Switching Protocols\r\nUpGrAdE: WebSocket\r\nConnection: keep-alive, Upgrade\r\nSec-WebSocket-Accept: {accept}\r\n\r\n', frame)
        websocket=_WebSocket(url,'test'); self.addCleanup(websocket.close)
    ...
    def test_constructor_rejects_each_required_upgrade_header(self):
        ...
        for response in responses:
            with self.subTest(response=response):
                with self.assertRaises(TransportError): _WebSocket(self.handshake_server(response),'test')
...
class TestLiveLifecycle(unittest.TestCase):
    ...
    def test_debug_disconnect_unblocks_receive_and_reconnects(self):
        ...
        self.manager._transition({'type':'Transition','startVersion':dict(self.manager.INITIAL_VERSION),'endVersion':{'querySet':1,'identity':0,'ts':'AQAAAAAAAAA='},'modifications':[{'type':'QueryUpdated','queryId':subscription.query_id,'value':{'count':0},'logLines':[]}]})
        update=subscription.next_update(.2)
        self.assertIsNone(update.error); self.assertEqual(update.value, {'count':0})
