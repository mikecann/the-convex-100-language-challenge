...
unittest
{
    auto result = decodeResponse("query",
            `{"status":"success","value":{"count":1},"logLines":["demo:state"]}`);
...
unittest
{
    ...
    try
    {
        decodeResponse("query",
                `{"status":"error","errorMessage":"room failed","errorData":{"code":"ROOM_EMPTY"},"logLines":["failure"]}`);
    }
    catch (ConvexError error)
    {
        failed = error.kind == "FunctionError"
            && error.data.object["code"].str == "ROOM_EMPTY" && error.logs == [
                "failure"
        ];
    }
    assert(failed);
