 ...
 * ordinary RFC6455 upgrade and frame rules. Convex messages, subscriptions,
 * query-set state, and reconnect policy are authored in LOLCODE. */
struct lol_websocket {
  CURL *curl;
...
#define LOL_WS_MAX_MESSAGE_BYTES (2U * 1024U * 1024U)
...
static int wait_for_socket(struct lol_websocket *websocket, short events,
                           int64_t deadline) {
  for (;;) {
...
static void retire_websocket(struct lol_websocket *websocket) {
...
long long lol_monotonic_millis(void) {
  return monotonic_millis();
}

static int send_raw(struct lol_websocket *websocket, const void *data, size_t length,
                    int64_t deadline) {
...
static int receive_raw(struct lol_websocket *websocket, void *output, size_t length,
...
static int perform_websocket_upgrade(struct lol_websocket *websocket, const char *url,
                                     long timeout_millis) {
...
void *lol_ws_connect(const char *url, const char *ca_path, long timeout_millis) {
  ...
  struct lol_websocket *websocket = calloc(1, sizeof *websocket);
  if (!websocket) {
    curl_easy_cleanup(curl);
    return NULL;
  }
...
int lol_ws_is_active(void *opaque) {
  struct lol_websocket *websocket = opaque;
  return websocket && websocket->active;
}

static int send_frame(struct lol_websocket *websocket, unsigned char opcode,
                      const void *data, size_t length, long timeout_millis) {
  if (!websocket || !websocket->active || timeout_millis <= 0 ||
      ((opcode & 8) && length > 125) || length > LOL_WS_MAX_MESSAGE_BYTES) return 0;
  ...
  *length = 0;
  ...
  for (;;) {
    ...
    if (!control && (payload_length > capacity - *length ||
                     payload_length > LOL_WS_MAX_MESSAGE_BYTES - *length)) break;
    consumed = 0;
...
/* Adapter multiplexing remains LOLCODE-owned. This exposes only POSIX
 * readiness so the owner can service frames while waiting for NDJSON input. */
...
/* TCP mode is an adapter I/O transport only. Once connected, LOLCODE reads and
...
int lol_adapter_listen(const char *address) {
  ...
  close(peer);
  /* The LOLCODE owner polls fd 0 between NDJSON lines. Disable stdio read-ahead so
