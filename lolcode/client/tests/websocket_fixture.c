...
int lol_ws_is_active(void *websocket);
int lol_ws_send_text(void *websocket, const char *data, size_t length, long timeout_millis);
int lol_ws_receive_text(void *websocket, char *output, size_t capacity,
                       size_t *length, long timeout_millis);
int lol_ws_close(void *websocket, long timeout_millis);
void lol_ws_free(void *websocket);

enum server_mode { HAPPY, PARTIAL_FRAME, STALLED };

static void fail(const char *message) {
  fprintf(stderr, "LOLCODE websocket fixture: %s\n", message);
  exit(1);
}
...
static void test_happy_path(void) {
  ...
  void *websocket = lol_ws_connect(url, "/tmp/ws-ca.pem", 1000);
  if (!websocket || !lol_ws_is_active(websocket)) fail("verified connect failed");
  if (!lol_ws_send_text(websocket, "hello", 5, 500)) fail("text send failed");
  char message[64] = {0};
  size_t length = 0;
  if (lol_ws_receive_text(websocket, message, sizeof message, &length, 500) != 1 ||
      length != 15 || memcmp(message, "frag \xe2\x98\x83 mented", 15))
    fail("fragmented text or ping/pong failed");
  if (!lol_ws_close(websocket, 250)) fail("close send failed");
  lol_ws_free(websocket);
...
static void test_tls_rejections(void) {
  ...
  void *websocket = lol_ws_connect(url, "/tmp/ws-ca.pem", 500);
  if (websocket) fail("hostname mismatch was accepted");
  wait_for_server(child);

  port = start_server(STALLED, &child);
  snprintf(url, sizeof url, "wss://localhost:%d/live", port);
  websocket = lol_ws_connect(url, "/etc/ssl/certs/ca-certificates.crt", 500);
...
static void test_partial_frame_retirement(void) {
  ...
  char message[64];
  size_t length = 0;
  if (lol_ws_receive_text(websocket, message, sizeof message, &length, 100) != -1 ||
      lol_ws_is_active(websocket))
...
static void test_idle_timeout_and_bounded_close(void) {
  ...
  char message[16];
  size_t length = 0;
  if (lol_ws_receive_text(websocket, message, sizeof message, &length, 75) != 0 ||
      !lol_ws_is_active(websocket))
    fail("idle timeout damaged connection");
  int64_t start = monotonic_millis();
  lol_ws_close(websocket, 100);
...
int main(void) {
  ...
  puts("lolcode websocket transport fixture: ok");
  return 0;
}
