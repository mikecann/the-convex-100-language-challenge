...
class TransportError extends Error { public function __construct(string $m, public string $operation = 'transport') { parent::__construct("Convex $operation transport error: $m"); } }
class FunctionError extends Error { public function __construct(string $m, public string $operation, public mixed $data = null, public array $logs = []) { parent::__construct("Convex $operation failed: $m"); } }
final class Result { public function __construct(public mixed $value, public array $logs = []) {} }
final class Update { public function __construct(public mixed $value = null, public ?Error $error = null, public array $logs = []) {} }

final class Client {
  private string $url; private string $token = ''; private bool $closed = false; private ?LiveManager $live = null;
  public function __construct(string $url, ?string $bearerToken = null, private string $version = 'php-0.1.0') {
    $p = parse_url($url); if (!is_array($p) || !isset($p['scheme'],$p['host']) || !in_array($p['scheme'], ['http','https'], true) || isset($p['user'])) throw new \InvalidArgumentException('Convex deployment URL must be http(s) with a host');
    $this->url = rtrim($url, '/'); $this->token = (string)$bearerToken;
  }
  public function setAuth(string $token): void { $this->guard(); $this->token = $token; }
  public function query(string $path, array $args = []): Result { return $this->call('query',$path,$args); }
  public function mutation(string $path, array $args = []): Result { return $this->call('mutation',$path,$args); }
  public function action(string $path, array $args = []): Result { return $this->call('action',$path,$args); }
  public function subscribe(string $path, array $args = []): Subscription { $this->validate($path,$args); $this->guard(); return ($this->live ??= new LiveManager($this->url,$this->version))->subscribe($path,$args); }
  public function pump(float $timeout = 0.0): void { $this->live?->pump($timeout); }
  public function debugDisconnectForAdapter(): void { $this->guard(); if (!$this->live) throw new TransportError('Live WebSocket is not connected','live'); $this->live->debugDisconnect(); }
  public function close(): void { if (!$this->closed) { $this->closed=true; $this->live?->close(); } }
  private function guard(): void { if ($this->closed) throw new ClosedError('Convex client is closed'); }
  private function validate(string $path, array $args): void { if ($path === '') throw new \InvalidArgumentException('Convex function path is required'); try { json_encode($args, JSON_THROW_ON_ERROR|JSON_INVALID_UTF8_SUBSTITUTE); } catch (\JsonException $e) { throw new \InvalidArgumentException('encode Convex arguments: '.$e->getMessage()); } }
  private function call(string $op, string $path, array $args): Result {
    $this->guard(); $this->validate($path,$args); $body=json_encode(['path'=>$path,'args'=>$args,'format'=>'json'],JSON_THROW_ON_ERROR|JSON_INVALID_UTF8_SUBSTITUTE);
    $headers="Content-Type: application/json\r\nAccept: application/json\r\nConvex-Client: {$this->version}\r\n".($this->token===''?'':"Authorization: Bearer {$this->token}\r\n");
    $ctx=stream_context_create(['http'=>['method'=>'POST','header'=>$headers,'content'=>$body,'timeout'=>30,'ignore_errors'=>true]]);
    $raw=@file_get_contents("{$this->url}/api/$op",false,$ctx); if ($raw===false) throw new TransportError(error_get_last()['message'] ?? 'HTTP request failed',$op); if (strlen($raw)>2097152) throw new TransportError('response exceeds 2097152 bytes',$op);
    try { $decoded=json_decode($raw,true,512,JSON_THROW_ON_ERROR); } catch (\JsonException $e) { throw new TransportError('HTTP response was not JSON: '.$e->getMessage(),$op); }
    if (($decoded['status']??null)==='success' && array_key_exists('value',$decoded)) return new Result($decoded['value'], array_values($decoded['logLines']??[]));
    if (($decoded['status']??null)==='error') throw new FunctionError((string)($decoded['errorMessage']??''),$op,$decoded['errorData']??null,array_values($decoded['logLines']??[]));
...
final class Subscription {
  ...
  private array $updates=[]; private bool $finished=false;
  public function __construct(private LiveManager $manager, public int $id) {}
  public function nextUpdate(float $timeout=10): Update { $end=microtime(true)+$timeout; while (!$this->updates && !$this->finished) { $this->manager->pump(max(0.0,min(.1,$end-microtime(true)))); if (microtime(true)>=$end) throw new TransportError('timed out waiting for Live update','live'); } if (!$this->updates) throw new ClosedError('Live subscription is closed'); return array_shift($this->updates); }
  public function deliver(Update $u): void { if (!$this->finished) { if (count($this->updates)===16) array_shift($this->updates); $this->updates[]=$u; } }
  public function close(): void { if (!$this->finished) $this->manager->unsubscribe($this->id); }
  public function finish(): void { $this->finished=true; $this->updates=[]; }
}

final class LiveManager {
  private ?WebSocket $ws=null; private array $subs=[]; private int $next=0,$querySet=0,$connections=0; private array $remote=['querySet'=>0,'identity'=>0,'ts'=>'AAAAAAAAAAA=']; private float $reconnectAt=0,$backoff=.1; private string $lastClose='InitialConnect'; private bool $closed=false;
  ...
  private function modify(array $mods):void { if(!$this->ws)return; $this->ws->send(['type'=>'ModifyQuerySet','baseVersion'=>$this->querySet,'newVersion'=>$this->querySet+1,'modifications'=>$mods]);$this->querySet++; }
  private static function add(int $id,array $s):array{return ['type'=>'Add','queryId'=>$id,'udfPath'=>$s['path'],'args'=>[$s['args']]];}
  private function handle(string $raw):void { try{$m=json_decode($raw,true,512,JSON_THROW_ON_ERROR);}catch(\JsonException $e){throw new ProtocolError('decode server message: '.$e->getMessage());} if(($m['type']??'')!=='Transition'){if(in_array($m['type']??'', ['Ping','MutationResponse','ActionResponse'],true))return;throw new ProtocolError('unknown server message '.($m['type']??''));} if(($m['startVersion']??null)!=$this->remote)throw new ProtocolError('Transition start version mismatch'); $changed=[];foreach(($m['modifications']??[]) as $x){$id=$x['queryId'];if(($x['type']??'')==='QueryUpdated')$changed[$id]=new Update($x['value']??null,null,array_values($x['logLines']??[]));elseif(($x['type']??'')==='QueryFailed')$changed[$id]=new Update(null,new FunctionError((string)($x['errorMessage']??''),'query',$x['errorData']??null,array_values($x['logLines']??[])),array_values($x['logLines']??[]));} $this->remote=$m['endVersion'];foreach($changed as $id=>$u)if(isset($this->subs[$id]))$this->subs[$id]['subscription']->deliver($u); }
  private function disconnect(string $reason):void { $this->ws?->close();$this->ws=null;$this->connections++;$this->lastClose=$reason;$this->querySet=0;$this->remote=['querySet'=>0,'identity'=>0,'ts'=>'AAAAAAAAAAA='];$this->reconnectAt=microtime(true)+$this->backoff;$this->backoff=min(15,$this->backoff*2); }
