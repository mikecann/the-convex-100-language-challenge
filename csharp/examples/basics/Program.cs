...
var before = Count((await client.Query("demo:state", roomArgs)).Value, "current query");
...
var applied = mutation.Value["applied"]?.GetValue<bool>() ?? false;
if (!applied) throw new InvalidOperationException("mutation was not applied");
var after = Count(mutation.Value["state"]!, "mutation");
