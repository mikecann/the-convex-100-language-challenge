...
static void Equal(int expected, int actual, string message) { if (expected != actual) throw new Exception(message); }
static void Reject(JsonNode state, string message)
{
    try { CountValue.Read(state, "test"); throw new Exception(message); }
...
Equal(int.MinValue, CountValue.Read(State(int.MinValue.ToString()), "minimum"), "Int32 minimum was rejected");
Equal(int.MaxValue, CountValue.Read(State(int.MaxValue.ToString()), "maximum"), "Int32 maximum was rejected");
