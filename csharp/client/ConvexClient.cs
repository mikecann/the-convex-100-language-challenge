...
public sealed class ConvexClient : IDisposable
{
    ...
    {
        if (!Uri.TryCreate(deployment.TrimEnd('/'), UriKind.Absolute, out var uri) ||
    ...
    public void SetAuth(string? token) { EnsureOpen(); _token = token ?? ""; }
    ...
    private async Task<Result> Call(string operation, string path, JsonObject args)
    {
        ...
        JsonObject decoded;
        try { decoded = (await response.Content.ReadFromJsonAsync<JsonObject>()) ?? throw new Exception("empty response"); }
