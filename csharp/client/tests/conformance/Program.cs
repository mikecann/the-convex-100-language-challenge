...
public static class Program
{
    ...
    private sealed class LockedWriter(TextWriter target){private readonly SemaphoreSlim gate=new(1,1);public async Task Write(JsonObject value){await gate.WaitAsync();try{await target.WriteLineAsync(Serialize(value));}finally{gate.Release();}}}
}
