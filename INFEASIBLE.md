...
| hack | HHVM reaches a TCP socket but cannot complete TLS against the real deployment, and there is no escape hatch. Its only in-language TLS mechanism, the PHP-derived `stream_socket_client('ssl://…')` / `stream_socket_enable_crypto()` API, fails the handshake against the hosted backend under five distinct context configurations (default, explicit SNI/crypto_method, verification disabled, explicit modern ciphers, TLS1.2-only ciphers), always with `ssl3_read_bytes:sslv3 alert handshake failure`; the same container's raw `openssl s_client` succeeds (TLS 1.3, ECDSA certificate), and forcing it to RSA-only ciphers reproduces the identical alert — HHVM's wrapper never offers an ECDSA-compatible ClientHello and its context options cannot override that. HHVM ships no FFI extension, so the raw-socket-plus-OpenSSL route is closed. Two further findings compound it: both public distribution channels (Docker Hub `hhvm/hhvm` and the `dl.hhvm.com` apt repo) stopped publishing in March 2023 — the newest image carries EOL Ubuntu 20.04 and EOL OpenSSL 1.1.1f with no maintained install path — and the Hack compiler is not separable from the runtime: there is no standalone `hackc`; the one `hhvm` binary any runtime image must ship still exposes the general-purpose `--hphp` compiler mode even in a trimmed image, which fails the no-compiler runtime rule and cannot be stripped without an upstream rebuild. Probe evidence on branch `codex/hack-feasibility` under `hack/feasibility/`. Replaced by Icarus Verilog. |
...
## Infeasible — reaches a real socket, but TLS is walled off by the toolchain's own tooling

Unlike every entry above, this one is not a language-design or licensing wall.
Two of its three gates passed outright, with committed, reproducible evidence.

| Language | Why it cannot be done here |
| --- | --- |
| oz | `Open.socket` is a genuine Mozart 2 built-in and completed a real HTTP round trip to a live host from inside a from-source Docker build (five modern-toolchain incompatibilities in the 2016-era build system found and fixed along the way — dead git submodule protocols, GCC 12 header changes, obsolete Boost/JVM flags, a Scala 2.10.4 bootstrap needing a fetched Temurin 8). TLS is the wall: `oztool`, the "DLL Builder" the language's own documentation describes for exactly this, is a Mozart *1.x* tool that does not exist anywhere in Mozart 2 — verified by direct grep of the full v2.0.1 source tree. The real extension path is VM-source-level: add a native builtin and rebuild the VM proper. That was attempted honestly. A `TLSConnection` class over `boost::asio::ssl::stream` (certificate chain and hostname verification both on) was written and wired into `ModOS` as four new builtins, needing five further fixes to Mozart 2's own unmaintained 2016 AST-dump code generator to even compile against Debian's only available Clang (an 8-major-version gap). The generator then compiled and linked, and `clang++` produced a valid AST file — but `generator intfimpl <ast> <outdir>` crashed on the specific header that carries the new builtins (`boostenv.hh`), always at the identical point: exactly 90 output files, always stopping at the `Chunk` declaration. Because one earlier, unrelated run against a *different* header had completed cleanly, a bounded retry loop was tried on the theory the crash was one-shot and address-layout-sensitive rather than deterministic — 200 attempts through the project's real `make genboostsources` target, plus 8 further manual attempts under `gdb` and varied stack limits: **208 consecutive failures, zero variation, same file every time.** That fixed point is a genuine, reproducible memory-safety defect in Mozart 2's own ~1000-line, long-unmaintained AST-walking tool, not bad luck. `setarch -R` to disable ASLR was correctly not attempted — Docker's default seccomp rejects `personality(ADDR_NO_RANDOMIZE)`, the identical mechanism that already ruled out Pop-11 above. Replaced by Modula-3, which reaches TLS through its own long-stable, actively-interoperable C FFI rather than through 2016-era vendored LLVM tooling. |
...
| 16 | gml | ~~Oz (1995)~~ **Modula-3** — see the Oz entry below; Oz proved infeasible on its own transport, not on any defect of GameMaker's. |
...
Hack's slot, vacated on a TLS stack that cannot be reached around, is filled
by:

- **Verilog** (Gateway Design Automation, 1984) via Icarus Verilog — the other
  hardware description language, and a genuinely different lineage from VHDL's:
  commercial and C-flavoured where VHDL was DoD-commissioned and Ada-flavoured,
  the two of them the reason chip designers still argue. `iverilog` is a plain
  Debian package, and the socket route is not speculative: Verisocks is an
  existing VPI module that gives an Icarus-simulated design a real TCP socket,
  which is the same foreign-boundary shape the VHDL entry already uses through
  VHPIDIRECT. Chosen over Modula-3, which is technically sound but would have
  been a third Wirth-family entry after modula-2 and oberon, and whose
  bootstrap needs a per-architecture prebuilt compiler.
