...
public final class LiveClientTest {
  ...
  private static void boundedDeliveryKeepsNewestValues() throws Exception {
    LiveClient.Subscription subscription =
        new LiveClient.Subscription(null, 7, "demo:state", ConvexClient.JSON.createObjectNode());
    for (int count = 0; count < 20; count++)
      subscription.offer(
          new LiveClient.Update(
              ConvexClient.JSON.createObjectNode().put("count", count), null, List.of()));
    check(
        subscription.next(Duration.ofMillis(10)).path("count").asInt() == 4,
        "buffer was not bounded to newest 16");
    for (int expected = 5; expected < 20; expected++)
      check(
          subscription.next(Duration.ofMillis(10)).path("count").asInt() == expected,
  ...
  private static void reconnectRestoresActiveQuery() throws Exception {
    try (ServerSocket listener = new ServerSocket(0)) {
          ...
          new Thread(
              () -> {
                try {
                  ...
                  long delayMillis =
                      TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - disconnectNanos.get());
      ...
      try (LiveClient live = new LiveClient("http://127.0.0.1:" + listener.getLocalPort());
          LiveClient.Subscription subscription =
              live.subscribe(
                  "demo:state", ConvexClient.JSON.createObjectNode().put("room", "reconnect"))) {
        check(
            subscription.next(Duration.ofSeconds(3)).path("count").asInt() == 0,
            "missing initial transition");
        disconnectNanos.set(System.nanoTime());
        live.debugDisconnect();
        check(
            subscription.next(Duration.ofSeconds(3)).path("count").asInt() == 1,
            "missing post-reconnect transition");
      }
      server.join(3_000);
      check(!server.isAlive(), "fake sync server did not finish");
      if (serverFailure[0] != null)
        throw new AssertionError("fake sync server failed", serverFailure[0]);
      check(reconnectDelayMillis.get() >= 75, "debug disconnect bypassed normal reconnect backoff");
    }
  }

  private static void serveConnection(Socket socket, int expectedConnectionCount, int count)
      throws Exception {
    try (socket) {
      ...
      check(
          connect.path("connectionCount").asInt() == expectedConnectionCount,
          "wrong connectionCount");
      JsonNode modify = ConvexClient.JSON.readTree(readClientText(input));
      check(
          "Add".equals(modify.path("modifications").path(0).path("type").asText()),
          "active query was not restored");
      int queryId = modify.path("modifications").path(0).path("queryId").asInt();
      String endTimestamp = count == 0 ? "AQAAAAAAAAA=" : "AgAAAAAAAAA=";
      JsonNode transition =
          ConvexClient.JSON.readTree(
              "{\"type\":\"Transition\",\"startVersion\":{\"querySet\":0,\"identity\":0,\"ts\":\"AAAAAAAAAAA=\"},\"endVersion\":{\"querySet\":1,\"identity\":0,\"ts\":\""
                  + endTimestamp
  ...
  private static String readHeaders(InputStream input) throws Exception {
    ...
    while (matched < 4) {
      ...
      if (value < 0) throw new IllegalStateException("EOF in handshake");
      bytes.write(value);
      matched =
          switch (matched) {
            case 0 -> value == '\r' ? 1 : 0;
  ...
  private static String header(String headers, String name) {
    for (String line : headers.split("\r\n")) {
      ...
      if (separator > 0 && line.substring(0, separator).toLowerCase(Locale.ROOT).equals(name))
  ...
  private static String readClientText(InputStream input) throws Exception {
    ...
    if (first < 0 || second < 0) throw new IllegalStateException("EOF in frame");
    int length = second & 0x7f;
    if (length == 126) length = (input.read() << 8) | input.read();
    else if (length == 127) {
      long large = 0;
      for (int i = 0; i < 8; i++) large = (large << 8) | input.read();
