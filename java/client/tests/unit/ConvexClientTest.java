...
public final class ConvexClientTest {
  public static void main(String[] args) throws Exception {
    ...
    server.createContext("/api/query", exchange -> {
      String response = "{\"status\":\"error\",\"errorMessage\":\"empty\",\"errorData\":{\"code\":\"ROOM_EMPTY\"},\"logLines\":[\"checked\"]}";
      exchange.sendResponseHeaders(560, response.length());
      exchange.getResponseBody().write(response.getBytes(StandardCharsets.UTF_8));
      exchange.close();
    });
    server.start();
    try (ConvexClient client = new ConvexClient("http://127.0.0.1:" + server.getAddress().getPort())) {
      try { client.query("demo:requiresNonzero", ConvexClient.JSON.createObjectNode()); throw new AssertionError("error response became success"); }
