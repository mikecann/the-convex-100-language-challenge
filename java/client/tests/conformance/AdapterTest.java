...
public final class AdapterTest {
  public static void main(String[] args) throws Exception {
    ...
    JsonNode failed =
        Adapter.failure(
            ...
            new ConvexClient.FunctionException(
                ...
                List.of("checked")));
    check(
        "ROOM_EMPTY".equals(failed.path("error").path("data").path("code").asText()),
        "lost error.data");
    ...
    check(
        "room".equals(subscriptionFailure.path("subscriptionId").asText()), "lost subscriptionId");
    ...
    try (ServerSocket server = Adapter.bind("127.0.0.1:0")) {
      ...
      try (Socket controller = new Socket("127.0.0.1", server.getLocalPort());
          PrintWriter commands =
              new PrintWriter(controller.getOutputStream(), true, StandardCharsets.UTF_8);
          BufferedReader events =
              new BufferedReader(
                  new InputStreamReader(controller.getInputStream(), StandardCharsets.UTF_8))) {
        ...
        check(
            "ready".equals(ready.path("type").asText())
                && "java".equals(ready.path("language").asText()),
            "bad TCP hello");
