...
public final class ConvexClient implements AutoCloseable {
  ...
  public ConvexClient(String deploymentUrl) {
    ...
    if (!("http".equals(parsed.getScheme()) || "https".equals(parsed.getScheme())) || parsed.getHost() == null || parsed.getUserInfo() != null) {
  ...
  public void setAuth(String bearerToken) { ensureOpen(); token = bearerToken == null ? "" : bearerToken; }
  public Result query(String path, JsonNode args) throws Exception { return call("query", path, args); }
  public Result mutation(String path, JsonNode args) throws Exception { return call("mutation", path, args); }
  public Result action(String path, JsonNode args) throws Exception { return call("action", path, args); }
  ...
  private Result call(String operation, String path, JsonNode args) throws Exception {
    ...
    HttpRequest.Builder request = HttpRequest.newBuilder(deployment.resolve("/api/" + operation))
      ...
      .header("Accept", "application/json").header("Convex-Client", "java-0.1.0")
    ...
    catch (Exception e) { throw new TransportException(operation, e); }
    JsonNode decoded;
    try { decoded = JSON.readTree(response.body()); }
    catch (Exception e) { throw new TransportException(operation, new IllegalStateException("non-Convex HTTP response", e)); }
    ...
    if ("error".equals(decoded.path("status").asText()))
      throw new FunctionException(operation, decoded.path("errorMessage").asText("Convex function failed"), decoded.get("errorData"), logs(decoded));
    throw new ProtocolException("HTTP " + response.statusCode() + " response has unknown status");
  }

  private static List<String> logs(JsonNode n) { return n.path("logLines").isArray() ? JSON.convertValue(n.path("logLines"), JSON.getTypeFactory().constructCollectionType(List.class, String.class)) : List.of(); }
