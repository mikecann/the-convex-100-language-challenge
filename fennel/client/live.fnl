...
(fn valid-version? [version]
  (and (= (type version) :table) (= (type version.querySet) :number)
       (= (% version.querySet 1) 0) (= (type version.identity) :number)
...
(fn later-timestamp [left right]
  (if (not left) right (let [left-bytes (basexx.from_base64 left)
                             right-bytes (basexx.from_base64 right)]
                         ;; Timestamps are little-endian uint64 values in base64.
                         (var result left)
                         (var index 8)
                         (while (and (>= index 1) (= result left))
                           (if (> (string.byte right-bytes index)
                                  (string.byte left-bytes index))
                               (set result right))
                           (set index (- index 1)))
                         result)))
...
(fn Subscription.deliver [self update]
  (if (not self.finished)
      (do
        ;; The queue is deliberately bounded. The adapter also has its own byte
        ;; budget so a stopped controller cannot turn event count into memory use.
        (if (= (length self.updates) max-buffered-updates)
            (table.remove self.updates 1))
        (table.insert self.updates update)
        (: self.changed :signal 1))))

(fn Subscription.finish [self]
  (if (not self.finished)
      (do
        (set self.finished true) (set self.updates [])
        (: self.changed :signal 1))))

(fn Subscription.try-next-update [self]
  (if (> (length self.updates) 0) (table.remove self.updates 1) nil))
...
(fn Manager.modify [self modifications timeout]
  (let [next-version (+ self.query-set-version 1)
    ...
    (let [(ok err) (: self :send message timeout)]
      (if ok (do
               (set self.query-set-version next-version) true)
          (values nil err)))))
