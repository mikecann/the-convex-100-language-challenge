...
struct FunctionError : Error {
  Json data;
  std::vector<std::string> logs;

  FunctionError(std::string message, Json value, std::vector<std::string> lines)
      : Error(std::move(message)), data(std::move(value)),
        logs(std::move(lines)) {}
};
...
public:
  ...
  std::shared_ptr<Subscription> subscribe(const std::string &path,
...
#endif
private:
  Result call(const std::string &operation, const std::string &path,
              const Json &args);
