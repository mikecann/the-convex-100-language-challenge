...
server_frame() {
  ...
  if ((n<126)); then length_code=$n; write_byte "$n"; elif ((n<65536)); then length_code=126; write_byte 126; write_byte $((n>>8)); write_byte $((n&255));
  else length_code=127; write_byte 127; for i in 56 48 40 32 24 16 8 0; do write_byte $(((n>>i)&255)); done; fi
...
client_frame() {
  ...
  if [[ -n $first ]]; then
    raw=$(dd bs=1 count=1 status=none | od -An -v -tu1); values=($raw)
    ((${#values[@]}==1)) || return 3; second=${values[0]}
  else
    raw=$(dd bs=1 count=2 status=none | od -An -v -tu1); values=($raw)
    ((${#values[@]}==2)) || return 3; first=${values[0]}; second=${values[1]}
  fi
  n=$((second&127)); opcode=$((first&15))
  ((second&128)) || { echo 'client frame was not masked'; return 2; }
  if ((n==126)); then
    raw=$(dd bs=1 count=2 status=none | od -An -v -tu1); values=($raw)
    ((${#values[@]}==2)) || return 3; n=$((values[0]*256 + values[1]))
  elif ((n==127)); then
    raw=$(dd bs=1 count=8 status=none | od -An -v -tu1); values=($raw)
    ((${#values[@]}==8)) || return 3; n=0
    for value in "${values[@]}"; do n=$((n*256+value)); done
  fi
  raw=$(dd bs=1 count="$((n+4))" status=none | od -An -v -tu1); values=($raw)
  ((${#values[@]}==n+4)) || return 3
  masks=("${values[@]:0:4}"); bytes=("${values[@]:4}"); i=0
...
if [[ $request = GET\ /api/sync* ]]; then
  ...
  while modify=$(client_frame); do
    ...
    for modification_line in "${modification_lines[@]}"; do
      ...
      pong=$(client_frame); [[ $pong = fixture-ping ]]
      # Exercise the valid 64-bit frame-length path once with a >64 KiB Ping.
      if [[ ! -f $state_dir/large-sent ]]; then
        : >"$state_dir/large-sent"; padding=$(dd if=/dev/zero bs=1024 count=65 status=none | tr '\0' x)
      ...
      if [[ $path = fixture:continuous ]]; then
        ...
        prefix=${transition%%世界*}; split=$(( $(printf %s "$prefix" | wc -c | tr -d ' ') + 1 ))
        ...
        server_frame "$first_part" 1 0; server_frame "$second_part" 0 1
      fi
    done
