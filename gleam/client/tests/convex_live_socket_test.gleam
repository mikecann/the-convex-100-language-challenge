...
pub fn main() -> Nil {
  ...
  oversized_value_reports_failure_and_recovers()
  ...
  peer_close_is_answered_before_reconnect()
...
fn same_identifier_replacement_is_a_barrier() -> Nil {
  ...
  let assert Ok(GateHeld(release_replacement, replacement_id, _event)) =
    process.receive(gate, wait)
  check.equal_string(
    "the delayed removal leaves the replacement active",
    replacement_id,
    replacement,
  )
  process.send(release_replacement, Nil)
  check.equal_int(
    "the replacement receives its value after QueryRemoved",
    next_count(updates, replacement),
    5,
  )
...
/// The WebSocket limit is larger than the conservatively charged relay limit.
/// A value between those limits must become an observable error, and must not
/// mutate `last` before a later small value proves the subscription recovers.
fn oversized_value_reports_failure_and_recovers() -> Nil {
  let #(listener, port) = start_listener()
  let assert Ok(client) = convex.new(local_url(port))
  let updates = process.new_subject()
  let assert Ok(subscription) =
    convex.subscribe(client, "demo:state", room_args("oversized"), updates)
  let socket = accept_upgrade(listener)
  let #(_connect, buffer) = expect_message(socket, <<>>)
  let #(_modify, _buffer) = expect_message(socket, buffer)
  ...
  case next_event(updates, subscription) {
    LiveFailure(error) ->
      check.equal_string(
        "an undeliverable Live value is observable protocol failure",
        error.name,
        "ProtocolError",
      )
  ...
  check.equal_int(
    "a smaller value recovers after delivery overflow",
    next_count(updates, subscription),
    9,
  )
...
/// A peer Close is a two-way handshake. The client must echo a Close frame
/// before retiring the socket, then reconnect the still-active subscription.
fn peer_close_is_answered_before_reconnect() -> Nil {
  let #(listener, port) = start_listener()
  let assert Ok(client) = convex.new(local_url(port))
  let updates = process.new_subject()
  let assert Ok(_subscription) =
    convex.subscribe(client, "demo:state", room_args("peer-close"), updates)
  let first = accept_upgrade(listener)
  let #(_connect, buffer) = expect_message(first, <<>>)
  let #(_modify, buffer) = expect_message(first, buffer)

  let assert Ok(_) = convex_sys.send(first, <<0x88, 0x02, 0x03, 0xE8>>, wait)
  expect_close_reply(first, buffer, 1000)
  convex_sys.close(first)

  let second = accept_upgrade(listener)
  let #(_connect, buffer) = expect_message(second, <<>>)
  let #(_modify, buffer) = expect_message(second, buffer)

  let assert Ok(_) = convex.close(client)
  expect_close_reply(second, buffer, 1000)
  convex_sys.close(second)
  convex_sys.close(listener)
}
...
fn start_listener() -> #(Socket, Int) {
  let assert Ok(listener) = convex_sys.listen("127.0.0.1", 0)
...
fn expect_close_reply(socket: Socket, buffer: BitArray, expected_code: Int) -> Nil {
  case take_client_close(buffer) {
    ...
    Error(_) -> {
      let assert convex_sys.Received(chunk) = convex_sys.recv(socket, 0, wait)
      expect_close_reply(socket, <<buffer:bits, chunk:bits>>, expected_code)
    }
  }
}

fn take_client_close(buffer: BitArray) -> Result(Int, Nil) {
  case buffer {
    <<0x88, 1:size(1), length:size(7), rest:bits>> -> {
      use #(payload, _tail) <- result.try(take_masked_payload(length, rest))
      case payload {
        <<code:size(16)-unsigned-big-int, _reason:bits>> -> Ok(code)
        _ -> Error(Nil)
      }
    }
    _ -> Error(Nil)
...
fn take_masked(length: Int, rest: BitArray) -> Result(#(String, BitArray), Nil) {
  use #(bytes, tail) <- result.try(take_masked_payload(length, rest))
...
fn send_text(socket: Socket, text: String) -> Nil {
  ...
  let header = case length {
    _ if length < 126 -> <<0x81, length>>
    _ if length < 65_536 -> <<0x81, 126, length:size(16)-big-int>>
    _ -> <<0x81, 127, length:size(64)-big-int>>
  }
  let assert Ok(_) =
...
fn updated_with_blob(query_id: Int, count: Int, blob_bytes: Int) -> String {
  "{\"type\":\"QueryUpdated\",\"queryId\":"
  <> int.to_string(query_id)
  <> ",\"value\":{\"count\":"
  <> int.to_string(count)
  <> ",\"blob\":\""
  <> string.repeat("x", blob_bytes)
  <> "\"},\"logLines\":[]}"
}
