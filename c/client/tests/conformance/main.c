  ...
  while (line && fgets(line, 2097153, input)) {
    ...
    if (!strcmp(op, "hello")) {
      ...
      if (!json_object_object_get_ex(command, "protocolVersion", &version) ||
        ...
        json_object_object_add(event, "language", json_object_new_string("c"));
        json_object_object_add(
            event, "implementation",
            json_object_new_string("native-c17-libcurl-websocket-json-c"));
        json_object_object_add(event, "runtime",
                               json_object_new_string("C17 musl"));
        emit(&output, event);
      }
