...
sub whole_count {
  ...
  die "$operation count was not a whole number" unless defined $value && $value =~ /^-?\d+$/;
  return 0 + $value;
}
...
eval {
  ...
  my $current_count = whole_count($current->{value}{count}, 'current query');
  ...
  my $initial = $subscription->next_update(10); die $initial->{error} if $initial->{error};
  my $initial_count = whole_count($initial->{value}{count}, 'initial Live value');
  ...
  my $mutation = $client->mutation('demo:increment', { room => $room, language => 'perl', runId => join('-', 'perl', time, int(rand(1_000_000))) });
  die 'mutation was not applied' unless $mutation->{value}{applied};
  print "mutation applied: true\n";
  my $mutation_count = whole_count($mutation->{value}{state}{count}, 'mutation');
  ...
  my $changed = $subscription->next_update(10); die $changed->{error} if $changed->{error};
  my $changed_count = whole_count($changed->{value}{count}, 'updated Live value');
